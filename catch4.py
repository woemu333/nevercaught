...
class ErrorWebhookHandler:
    ...
    def write(self, message):
        ...
        if message.strip():  # Avoid sending empty messages
            if '[warning ]' not in message.lower() or 'selfcord.errors.InvalidData: Did not receive a response from Discord' not in message.lower():
...
error_handler = ErrorWebhookHandler(config['urls']['catch4'])
...
async def on_ready():
    global servers
    servers = list(config['servers7']) + list(config['servers8'])
    task1.start()
...
client.run(config['tokens']['catch4'])