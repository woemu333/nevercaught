...
async def _giveall(ctx: commands.Context):
    if ctx.author.id == 707866373602148363:
        ...
        newstorage = await bot.fetch_user(1273823384278532128)
        ...
        while True:
            ...
            # send list command
            list = await listcommand.__call__(sort='catch_date')

            await asyncio.sleep(4)

            if "You don't have any countryballs yet" in list.message.content:
                await ctx.send(f'<@707866373602148363> gave {count} balls to new account!')
            
            for ball in list.message.components[2].children[0].options:

                hexid = ball.label.split("#")[1].split(" ")[0]
                decid = int(hexid, 16)

                give = await givecommand.__call__(channel=ctx.channel, user=newstorage, countryball=decid)
                count+=1
                print(count)
